...
export const MapScreen = () => {
  ...
  const handleLockTime = async (timeSlot: TimeSlot) => {
    ...
    try {
      ...
      // Cria a data no formato ISO (Apenas para o teste, estamos a assumir o dia de hoje)
      ...
      await apiClient.lockTable({
        ...
        clientId: 'cliente-mobile-123' // No futuro, este será o ID real do utilizador
      });

      Alert.alert(
        "Sucesso!", 
        "Mesa reservada temporariamente. Tem 5 minutos para confirmar os seus dados.",
        [{ text: "Prosseguir", onPress: () => console.log("Navegar para o Checkout") }]
      );
      
      setSelectedTable(null); // Fecha o modal
      loadSectorData(); // Recarrega os dados para a mesa ficar cinzenta (ocupada)
      
    } catch (error: any) {
      Alert.alert("Ops!", error.message || "Este horário acabou de ser reservado por outra pessoa.");
    } finally {
      setIsLocking(false);
    }
  };
